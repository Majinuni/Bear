...
with urllib.request.urlopen("https://kenya-rapid-app-server-dev.mybluemix.net/waterpoints") as url:
...
def get_waterpoints(county=None):
    ...
    for point in data["data"]:
        waterpoints[point["id"]] = {"name": point["name"], 
        ...
        if "waterpointCost" in point:
            waterpoints[point["id"]]["waterpointCost"] = point["waterpointCost"]
        else:
        ...
        if "distancefromcountycenter" in point:
            waterpoints[point["id"]]["distancefromcountycenter"] = point["distancefromcountycenter"]
        else:
            waterpoints[point["id"]]["distancefromcountycenter"] = "unavailable"

        # keeps water point usage for the waterpoints that have them available
        if "waterpointUsage" in point:
            waterpoints[point["id"]]["waterpointUsage"] = point["waterpointUsage"]
        else:
